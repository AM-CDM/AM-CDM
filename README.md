...
When referencing the SADL files, Classes are sets of things (or data object types) such as PowderStock, MaterialStockInput, ManufacturingProcessStep, ProcessHistory, and Specimen that appear as the tope level element.  They can either be their own Class, or a Child Class when identified as "type of <Parent Class>".  Properties are attributes of the classes they belong to, and are identified in the files by the line "described by" under Classes.  Properties are represented in a dataset by a value (or values when multuple are allowed) of a type.  Native types include string, double, int, and dateTime.  All other types are defined in the SADL files and are used to connect data objects.  In order to adopt all or a portion of the CDM to a data set it is important to explain in more detail some characteristics of the data model:
